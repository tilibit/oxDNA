...
def parse_header(e_txt: str) -> List[str]:
    ...
    e_list = e_txt.split(" ")[
        2:
    ]  # The first two are id1 and id2 (unless somebody writes a non-pairwise potential, then it's your problem)
    return e_list
...
def cli_parser(prog="output_bonds.py"):
    ...
    parser.add_argument(
        ...
        nargs=1,
        dest="outfile",
        help="if you want instead average per-particle energy as an oxView JSON",
    )
    parser.add_argument(
        "-p",
        "--parallel",
    ...
    parser.add_argument(
        "-u", "--units", type=str, nargs=1, dest="units", help="(optional) The units of the energy (pNnm or oxDNA)"
    )
    parser.add_argument(
        "-q",
        "--quiet",
