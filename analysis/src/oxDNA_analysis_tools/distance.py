...
ComputeContext = namedtuple("ComputeContext", ["traj_info", "top_info", "p1s", "p2s"])
...
def compute(ctx: ComputeContext, chunk_size: int, chunk_id: int):
    ...
    for i, conf in enumerate(confs):
        distances[:, i] = [
            min_image(conf.positions[p1], conf.positions[p2], box) * 0.85 for p1, p2 in zip(ctx.p1s, ctx.p2s)
        ]
...
def distance(
    traj_infos: List[TrajInfo], top_infos: List[TopInfo], p1ss: List[List[int]], p2ss: List[List[int]], ncpus: int = 1
) -> List[List[float]]:
    """
    ...
    for i, (traj_info, top_info, p1s, p2s) in enumerate(zip(traj_infos, top_infos, p1ss, p2ss)):
        ...
        logger.info(f"Working on trajectory: {traj_info.path}")
...
def cli_parser(prog="distance.py"):
    ...
    parser = argparse.ArgumentParser(
        prog=prog, description="Finds the ensemble of distances between any two particles in the system"
    )
    parser.add_argument(
        "-i",
        "--input",
        metavar=("input p1 p2 (p1 p2...)"),
        nargs="+",
        action="append",
        help="A trajectory, and a list of particle pairs to compare.  Can call -i multiple times to plot multiple datasets.",
    )
    ...
    parser.add_argument(
        ...
        metavar="data_file",
        nargs=1,
        help="If set, the output for the graphs will be dropped as a json to this filename for loading in oxView or your own scripts",
    )
    parser.add_argument(
        "-n",
        "--names",
        ...
        nargs="+",
        action="append",
        help="Names of the data series.  Will default to particle ids if not provided",
    )
    parser.add_argument(
        "-p",
        "--parallel",
...
def main():
    ...
    if args.output:
        ...
        logger.info("No outfile name provided, defaulting to distance.png")
    ...
    if args.names:
        ...
        if len(names) < n_dists:
            logger.info(
                "Names list too short.  There are {} items in names and {} distances were calculated.  Will pad with particle IDs".format(
                    len(names), n_dists
                )
            )
            for i in range(len(names), len(distances)):
                names.append(f"{[j for sl in p1ss for j in sl][i]}-{[j for sl in p2ss for j in sl][i]}")
        if len(names) > n_dists:
            logger.info(
                "Names list too long. There are {} items in names and {} distances were calculated.  Truncating to be the same as distances".format(
                    len(names), n_dists
                )
            )
            names = names[:n_dists]
        ...
        names = [f"{p1}-{p2}" for p1, p2 in zip([i for sl in p1ss for i in sl], [i for sl in p2ss for i in sl])]
    ...
    if args.data:
        ...
        if len(trajectories) > 1:
            logger.info(
                "distance lists from separate trajectories are printed to separate files for oxView compatibility.  Trajectory numbers will be appended to your provided data file name."
            )
        ...
        names_by_traj = [[f"{p1}-{p2}" for p1, p2 in zip(p1l, p2l)] for p1l, p2l in zip(p1ss, p2ss)]
    ...
    print("input:\t", end="")
    [print(f"{p1}-{p2}\t", end="") for p1, p2 in zip([i for sl in p1ss for i in sl], [i for sl in p2ss for i in sl])]
    print("")

    print("name:\t", end="")
    [print(f"{t}\t", end="") for t in names[:n_dists]]
    print("")

    print("mean:\t", end="")
    [print(f"{m:.2f}\t", end="") for m in [i for sl in means for i in sl]]
    print("")

    print("stdev:\t", end="")
    [print(f"{s:.2f}\t", end="") for s in [i for sl in stdevs for i in sl]]
    print("")

    print("median:\t", end="")
    [print(f"{m:.2f}\t", end="") for m in [i for sl in medians for i in sl]]
    print("")
    ...
    if hist == True:
        ...
        # plt.show()
        plt.tight_layout()
        logger.info(f"Writing histogram to file {out}")
        plt.savefig(f"{out}")
    ...
    if lineplt == True:
        ...
        # plt.show()
        plt.tight_layout()
        logger.info(f"Writing trajectory plot to file {out}")
        plt.savefig(f"{out}")
