...
def deviations(
    traj_info: TrajInfo, top_info: TopInfo, mean_conf: Configuration, indexes: List[int] = [], ncpus: int = 1
    ...
    ctx = ComputeContext(traj_info, top_info, mean_conf, indexes)
...
def output(
    RMSDs: np.ndarray,
    RMSFs: np.ndarray,
    outfile: str = "devs.json",
    plot_name: str = "rmsd.png",
    data_file: str = "rmsd_op.json",
):
    """
...
def cli_parser(prog="deviations.py"):
    ...
    parser = argparse.ArgumentParser(
        prog=prog, description="Compute the RMSD of each nucleotide from the mean structure produced by compute_mean.py"
    )
    ...
    parser.add_argument(
        "-o", "--output", metavar="output_file", nargs=1, help="The filename to save the RMSF json file to"
    )
    parser.add_argument(
        "-i",
        "--index",
        ...
        dest="index_file",
        nargs=1,
        help="Compute mean structure of a subset of particles from a space-separated list in the provided file",
    )
    parser.add_argument(
        "-r",
        "--plot",
    ...
    parser.add_argument(
        ...
        dest="rmsd_data",
        nargs=1,
        help="The name of the file to save the RMSD data in json format.",
    )
    parser.add_argument(
        "-q",
        "--quiet",
