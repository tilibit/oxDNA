...
from oxDNA_analysis_tools.UTILS.RyeReader import get_top_string
...
def compute(ctx: ComputeContext, chunk_size: int, chunk_id: int):
    ...
    for conf in confs:
        sub_confs = [
            Configuration(conf.time, conf.box, conf.energy, conf.positions[i], conf.a1s[i], conf.a3s[i])
...
def subset(
    traj_info: TrajInfo, top_info: TopInfo, system: System, indexes: List[List[int]], outfiles: List[str], ncpus=1
):
    """
...
def cli_parser(prog="subset_trajectory.py"):
    ...
    parser.add_argument(
        "-i",
        "--index",
        metavar=("index", "outfile"),
        action="append",
        nargs=2,
        help="A space separated index file and the associated output file name.  This can be called multiple times",
    )
    parser.add_argument(
        "-p", "--parallel", metavar="num_cpus", type=int, dest="parallel", help="(optional) How many cores to use"
    )
    parser.add_argument(
        "-f", "--old_format", action="store_true", dest="old_format", help="Use the old 3'-5' topology format?"
    )
    parser.add_argument(
        "-q",
        "--quiet",
    ...
    for i, o in zip(index_files, output_files):
        with open(i) as f:
            ...
            try:
                ...
                raise RuntimeError(
                    'The index file {} must be a space-seperated list of particles.  These can be generated using oxView by clicking the "Download Selected Base List" button'.format(
                        i
                    )
                )
        indexes.append(data)
