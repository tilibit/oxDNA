...
def decimate(traj: str, outfile: str, ncpus: int = 1, start: int = 0, stop: Union[int, None] = None, stride: int = 10):
    ...
    ctx = ComputeContext(my_di, top_info)
...
def cli_parser(prog="decimate.py"):
    parser = argparse.ArgumentParser(
        prog=prog, description="Creates a smaller trajectory only including start/stop/stride frames from the input."
    )
    ...
    parser.add_argument(
        "-p", "--parallel", dest="parallel", default=1, type=int, help="(optional) How many cores to use"
    )
    parser.add_argument(
        "-s", "--start", dest="start", default=0, type=int, help="First conf to write to the output file."
    )
    parser.add_argument(
        "-e",
        "--stop",
        ...
        default=None,
        type=int,
        help="Process up to this conf (exclusive).  Accepts negative indexes.",
    )
    parser.add_argument(
        "-d", "--stride", dest="stride", default=10, type=int, help="Write out every this many confs (default=10)"
    )
    parser.add_argument(
        "-q",
        "--quiet",
