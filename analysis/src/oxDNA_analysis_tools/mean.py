...
def mean(
    ...
    ctx = ComputeContext(traj_info, top_info, reference_coords, indexes)
...
def cli_parser(prog="mean.py"):
    ...
    parser.add_argument(
        "-o", "--output", metavar="output_file", nargs=1, help="The filename to save the mean structure to"
    )
    parser.add_argument(
        "-d", "--deviations", metavar="deviation_file", nargs=1, help="Immediately run oat deviations from the output"
    )
    parser.add_argument(
        "-i",
        "--index",
...
def main():
    ...
    if args.deviations:
        ...
        deviations.output(
            RMSDs, RMSFs, dev_file, dev_file.split(".")[0] + "_rmsd.png", dev_file.split(".")[0] + "_rmsd_data.json"
        )
