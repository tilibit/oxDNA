...
from oxDNA_analysis_tools.UTILS.pdb import PDB_Nucleotide
        ...
        for l in lines:
            if l.startswith("ATOM"):
                ...
                if l[17:20].strip() in na_pdb_names:
                    raise RuntimeError(
                        "Invalid residue name {} in {}. The reference PDB file must only contain protein residues.".format(
                            l[17:20].strip(), pdbfile
                        )
                    )
...
def peptide_to_pdb(
    strand: Strand, conf: Configuration, pdbfile: str, reading_position: int
) -> Tuple[int, List[PDB_AminoAcid]]:
    """
    ...
    """
    coord = np.array(
        [conf.positions[m.id] for m in strand.monomers]
    )  # amino acids only go from nterm to cterm (pdb format does as well)
...
def oxDNA_PDB(
    ...
    system: System,
    out_basename: str,
    protein_pdb_files: List[str] = [],
    reverse: bool = False,
    hydrogen: bool = True,
    uniform_residue_names: bool = False,
    one_file_per_strand: bool = False,
    rmsf_file: str = "",
):
    """
    ...
    if one_file_per_strand:
        out_name = out_basename + f"_{system.strands[0].id}.pdb"
    else:
            ...
            if strand.id < 0 and protein_pdb_files:
                ...
                if reading_position == -1:
                    try:
                        ...
                        protein_pdb_files = (
                            []
                        )  # had better be nucleic acids next or we're going to the error in the elif.
                ...
                raise RuntimeError(
                    "You must provide PDB files containing just the protein for each protein in the scene."
                )
...
def cli_parser(prog="oxDNA_PDB.py"):
    parser = argparse.ArgumentParser(
        prog=prog,
        description="Convert oxDNA files to PDB.  This converter can handle oxDNANM protein simulation files.",
    )
    parser.add_argument("topology", type=str, help="the oxDNA topology file for the structure")
    parser.add_argument("configuration", type=str, help="the configuration file you wish to convert")
    parser.add_argument(
        "direction",
        type=str,
    ...
    parser.add_argument(
        "-o", "--output", type=str, help="The name of the output pdb file.  Defaults to name of the configuration+.pdb"
    )
    parser.add_argument(
        "-d",
        "--output_direction",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="if you want to have one PDB file per strand",
    )
    parser.add_argument(
        "-r",
        "--rmsf-file",
