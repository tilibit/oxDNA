...
def get_angle_between(
    files: List[str], p1s: List[List[int]], p2s: List[List[int]], invert_mask: List[bool]
) -> Tuple[List[List[np.ndarray]], List[List[float]], List[List[float]], List[List[float]], List[List[float]]]:
    """
...
def cli_parser(prog="duplex_angle_plotter.py"):
    ...
    parser = argparse.ArgumentParser(
        prog=prog,
        description="Finds the ensemble of angles between pairs duplexes defined by nucleotides in the duplexes.",
    )
    parser.add_argument(
        "-i",
        "--input",
        ...
        nargs="+",
        action="append",
        help="An angle file from duplex_angle_finder.py and a list of particle id pairs found in the duplexes you want to compare.  Can call -i multiple times to plot multiple datasets together.",
    )
    parser.add_argument(
        "-v", "--invert_mask", dest="invert_mask", nargs="+", help="If 1 invert the i-th vector, if 0, do nothing."
    )
...
def main():
    ...
    if args.data:
        ...
        names_by_traj = [[f"{p1}-{p2}" for p1, p2 in zip(p1l, p2l)] for p1l, p2l in zip(p1s, p2s)]
    ...
    print("name:\t", end="")
    [print(f"{t}\t", end="") for t in names[:n_angles]]
    print("")
    ...
    print("freqs:\t", end="")
    [print(f"{r:.2f}\t", end="") for r in [i for sl in representations for i in sl]]
    print("")
