...
ComputeContext = namedtuple("ComputeContext", ["traj_info", "top_info", "input_file", "monomers"])
...
def find_duplex(monomers: List[Monomer]) -> List[Duplex]:
    ...
    assigned_monomers = set(
        {0}
    )  # set of IDs because objects are not hashable.  Start with 0 filled to create an initial duplex if paired.
...
def duplex_finder(
    traj_info: TrajInfo, top_info: TopInfo, inputfile: str, monomers: List[Monomer], ncpus=1
) -> List[List[Duplex]]:
    """
...
def cli_parser(prog="duplex_finder.py"):
    ...
    parser.add_argument(
        "-o", "--output", metavar="output_file", type=str, nargs=1, help="name of the file to write the angle list to"
    )
    parser.add_argument(
        "-q",
        "--quiet",
...
def main():
    ...
    logger.info(f"Writing duplex data to {outfile}.  Use duplex_angle_plotter to graph data")
    with open(outfile, "w") as f:
        ...
        for i in range(0, len(duplexes_at_step)):
            for j in range(0, len(duplexes_at_step[i])):
                line = "{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t[{},{},{}]\n".format(
                    i,
                    duplexes_at_step[i][j].index,
                    duplexes_at_step[i][j].start1,
                    duplexes_at_step[i][j].end1,
                    duplexes_at_step[i][j].start2,
                    duplexes_at_step[i][j].end2,
                    duplexes_at_step[i][j].axis[0],
                    duplexes_at_step[i][j].axis[1],
                    duplexes_at_step[i][j].axis[2],
                    duplexes_at_step[i][j].pos[0],
                    duplexes_at_step[i][j].pos[1],
                    duplexes_at_step[i][j].pos[2],
                )
                f.write(line)
