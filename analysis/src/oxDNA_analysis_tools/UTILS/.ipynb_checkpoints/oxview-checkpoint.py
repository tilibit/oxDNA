...
from oxDNA_analysis_tools.UTILS.RyeReader import TopInfo
...
def __compress_dat(conf):
    ...
    dat_string += "\\n".join(
        [
            f"{p[0]:.3f} {p[1]:.3f} {p[2]:.3f} {a1[0]:.8f} {a1[1]:.8f} {a1[2]:.8f} {a3[0]:.8f} {a3[1]:.8f} {a3[2]:.8f}"
...
def display_files(
    files_with_ext, inbox_settings=["Monomer", "Origin"], oxview_src="https://sulcgroup.github.io/oxdna-viewer/"
):
    """
    ...
    a(
        f"frame.contentWindow.postMessage({{message : 'iframe_drop',files: t_files, ext: t_ext, inbox_settings : {inbox_settings} }}, \"{oxview_src}\");"
    )
    a("}")
    a("</script>")
...
def loro_patchy_conf(
    ...
    file_list = [(top_string, "LORO.top"), (dat_string, "LORO.dat"), (matrix_string, "matrix")]

    display_files(file_list, inbox_settings, oxview_src)


def flro_patchy_conf(
    top_path: str,
    conf: Configuration,
    particles_path: str,
    inbox_settings=["Monomer", "Origin"],
    ...
    # locationr to store the things we want to past to js
    file_list = [(top_string, "top"), (dat_string, "dat"), (particles_string, "particles")]
    display_files(file_list, inbox_settings, oxview_src)
