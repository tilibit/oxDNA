...
FROM_ANGSTROM_TO_OXDNA = 1.0 / FROM_OXDNA_TO_ANGSTROM
...
class Atom:
    ...
    def to_pdb(self, residue_suffix: str, bfactor: float) -> Dict:
        return {"name": self.name, "residue_name": self.residue + residue_suffix, "pos": self.pos, "bfactor": bfactor}
    ...
    def to_mgl(self):
        ...
        for c in colors:
            if c in self.name:
                color = colors[c]
        r = 0.5
        return f"{self.pos[0]} {self.pos[1]} {self.pos[2]} @ {r:f} C[{color}]"


class PDB_Nucleotide:
...
class PDB_AminoAcid:
    ...
    def get_ca_pos(self):
        if self.ca_atom:
            ...
            raise RuntimeError(
                "No CA atom found in amino acid residue name:{}, id:{}, chain:{}".format(
                    self.name, self.idx, self.chain_id
                )
            )
