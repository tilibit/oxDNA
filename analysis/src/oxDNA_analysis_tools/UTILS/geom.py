...
def get_RNA_axis(particles, d):
    ...
    for i in range(len(posAs) - 1):
        ...
        if np.linalg.norm(midpointA + t * perpendicular_vecA - (midpointB + c * perpendicular_vecB)) > 1.0e-6:
            print(
                "Error in finding common intersection point",
                midpointA + t * perpendicular_vecA,
                midpointB + c * perpendicular_vecB,
            )
...
def get_DNA_axis(particles, d):
    ...
    for i, j in zip(range(d.start1, d.end1), range(d.end2, d.start2, -1)):
        nucA = particles[i]
        nucAc = particles[j]
        midA = (nucA.base_site() + nucAc.base_site()) / 2.0
        nucB = particles[i + 1]
        nucBc = particles[j - 1]
        midB = (nucB.base_site() + nucBc.base_site()) / 2.0
    ...
    pos = (particles[d.start1].base_site() + particles[d.end2].base_site()) / 2.0
