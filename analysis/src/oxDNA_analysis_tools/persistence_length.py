...
def persistence_length(
    traj_info: TrajInfo, inp_file: str, n1: int, n2: int, ncpus: int = 1
...
def cli_parser(prog="persistence_length.py"):
    parser = argparse.ArgumentParser(
        prog=prog, description="Calculates persistence length and contour length of a paired sequence of DNA."
    )
    ...
    parser.add_argument(
        ...
        metavar="data_file",
        nargs=1,
        help="If set, the correlations will be written to a txt file in the format `offset correlation`",
    )
    parser.add_argument(
        "-n",
        "--plot_name",
        nargs=1,
        help="Name to save the plot showing the fit of persistence length to correlations.  Defaults to persistence_length.png",
    )
    parser.add_argument(
        "-q",
        "--quiet",
...
def main():
    ...
    print(f"Persistence length: {pl:.1f} nucleotides")
