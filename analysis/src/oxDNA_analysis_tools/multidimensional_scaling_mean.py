...
DevsContext = namedtuple("DevsContext", ["traj_info", "top_info", "masked_mean_coords"])
...
def multidimensional_scaling_mean(
    traj_info: TrajInfo, top_info: TopInfo, ncpus: int = 1
    ...
    )  # without the init you can get a left-handed structure.
...
def cli_parser(prog="multidimensional_scaling_mean.py"):
    ...
    parser = argparse.ArgumentParser(
        prog=prog, description="Calculate molecular contacts, and assembles an average set of contacts based on MDS"
    )
