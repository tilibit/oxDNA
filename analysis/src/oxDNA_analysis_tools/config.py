...
def check(to_check: List[str] = ["python", "numpy", "matplotlib", "sklearn", "oxpy"]):
    ...
    real_names = {"numpy": "Numpy", "matplotlib": "MatPlotLib", "sklearn": "SciKit-Learn", "oxpy": "oxpy"}
    websites = {
        "numpy": "numpy.org",
    ...
    for package in to_check:
        ...
        try:
            ...
            print(
                "ERROR: Unable to find package {}.  Please check your environment or follow the installation instructions at {}".format(
                    real_names[package], websites[package]
                ),
                file=stderr,
            )
            continue
    ...
    try:
        ...
        flag = True
        raise RuntimeError(
            f"Importing Cython file reading module failed with error:\n{e}\n\
...
def cli_parser(prog="config.py"):
    ...
    parser.add_argument(
        "-n", "--chunk_size", type=int, help="Number of configurations to per chunk.  Persistent across analyses."
    )
    parser.add_argument(
        "-q",
        "--quiet",
