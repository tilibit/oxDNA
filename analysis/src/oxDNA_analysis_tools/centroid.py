...
def compute_centroid(ctx: ComputeContext, chunk_size, chunk_id: int) -> Tuple[np.ndarray, float, int]:
    ...
    for i, c in enumerate(np_confs):
        c[0] -= np.mean(
            c[0][ctx.indexes], axis=0
        )  # didn't center earlier because you have to center on the indexed particles
        aligned_conf = svd_align(ctx.ref_coords.positions[ctx.indexes], c, ctx.indexes, ref_center=np.zeros(3))[0]
        RMSD = np.sqrt(
            np.mean(np.linalg.norm(aligned_conf[ctx.indexes] - ctx.ref_coords.positions[ctx.indexes], axis=1) ** 2)
        )
        if RMSD < min_RMSD:
            min_RMSD = RMSD
...
def centroid(
    traj_info: TrajInfo, top_info: TopInfo, ref_conf: Configuration, indexes: List[int] = [], ncpus=1
    ...
    ctx = ComputeContext(traj_info, top_info, ref_conf, indexes)

    # What do we do with the output from the worker processes?
    min_RMSD = np.inf
    centroid_candidate = Configuration(
        0,
        ref_conf.box,
        np.zeros(3),
        np.zeros_like(ref_conf.positions),
        np.zeros_like(ref_conf.positions),
        np.zeros_like(ref_conf.positions),
    )
    centroid_time = -1
...
def cli_parser(prog="centroid.py"):
    ...
    parser = argparse.ArgumentParser(
        prog=prog, description="Find the configuration in a trajectory closest to a provided reference configuration"
    )
    ...
    parser.add_argument(
        ...
        dest="index_file",
        nargs=1,
        help="Alignment and RMSD based on a subset of particles given in a space-separated list in the provided file",
    )
    parser.add_argument(
        "-q",
        "--quiet",
