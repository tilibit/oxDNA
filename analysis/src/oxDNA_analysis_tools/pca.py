...
def make_heatmap(covariance: np.ndarray):
    ...
    ax.set(title="interaction network", ylabel="nucleotide id*3", xlabel="nucleotide id*3")
...
def pca(
    traj_info: TrajInfo, top_info: TopInfo, mean_conf: Configuration, ncpus: int = 1
    ...
    ctx = ComputeContext_cov(traj_info, top_info, mean_conf.positions)
    ...
    while running < 0.9:
        ...
        i += 1
    print(f"90% of the variance is found in the first {i} components")
...
def cli_parser(prog="pca.py"):
    parser = argparse.ArgumentParser(
        prog=prog, description="Calculates a principal component analysis of nucleotide deviations over a trajectory"
    )
    parser.add_argument("trajectory", type=str, nargs=1, help="the trajectory file you wish to analyze")
    parser.add_argument("meanfile", type=str, nargs=1, help="The mean structure from oat mean")
    parser.add_argument(
        "outfile", type=str, nargs=1, help="the name of the oxView .json overlay file where the PCs will be written"
    )
    parser.add_argument(
        "-p",
        "--parallel",
    ...
    parser.add_argument(
        ...
        type=int,
        dest="N",
        help="(optional) Print the first N components as oxView overlay files (defaults to 1)",
    )
    parser.add_argument(
        "-q",
        "--quiet",
