...
sources = glob(f"src/oxDNA_analysis_tools/cython_utils/*.{ext}")
extensions = [
    Extension("oxDNA_analysis_tools.UTILS." + source.split(os.path.sep)[-1].split(".")[0], sources=[source])
...
def get_property(prop):
    ...
    result = re.search(rf'{prop}\s*=\s*[\'"]([^\'"]*)[\'"]', open(init_location).read())
    return result.group(1)


# invoke actual setup
setup(
    name="oxDNA-analysis-tools",
